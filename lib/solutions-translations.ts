export const solutionsTranslations = {
  es: {
    // Header
    home: "Inicio",
    solutions: "Soluciones",
    contactUs: "Contáctenos",

    // Hero
    heroTitle: "Soluciones Integrales de",
    heroTitleHighlight: "Carga EV",
    heroSubtitle:
      "De la instalación a las ventas, entregamos soluciones completas de infraestructura de carga para cada necesidad en América Latina.",

    // Installation Section
    installationTitle: "Instalación de Cargadores EV",
    installationSubtitle:
      "Servicios profesionales de instalación desde residencias hasta implementaciones comerciales a gran escala",

    residentialTitle: "Instalación Residencial",
    residentialDesc:
      "Instalación de cargadores domésticos para propietarios de VE. Soluciones plug-and-play con cableado profesional, certificación de seguridad y soporte continuo.",
    residentialFeature1: "Instalación en viviendas unifamiliares y multifamiliares",
    residentialFeature2: "Cargadores de Nivel 2 (7-22 kW)",
    residentialFeature3: "Permisos, certificaciones y garantías incluidos",

    commercialTitle: "Instalación Comercial",
    commercialDesc:
      "Estaciones de carga comerciales para empresas, flotas y lugares públicos. Soluciones escalables desde una sola estación hasta redes multi-sitio.",
    commercialFeature1: "Cargadores de nivel 2 y DC rápidos (hasta 350 kW)",
    commercialFeature2: "Evaluaciones de sitio y diseño eléctrico",
    commercialFeature3: "Integración de software de gestión de carga",

    turnkeyTitle: "Entrega Llave en Mano",
    turnkeyDesc:
      "Proyectos completos llave en mano desde la planificación hasta la operación. Manejamos permisos, instalación, puesta en marcha y capacitación del personal.",
    turnkeyFeature1: "Gestión completa del proyecto y cronograma",
    turnkeyFeature2: "Cumplimiento y certificación regulatoria",
    turnkeyFeature3: "Soporte post-instalación y mantenimiento",

    // Sales Section
    salesTitle: "Venta de Cargadores EV",
    salesSubtitle: "Acceso directo a cargadores EV de alta calidad para distribuidores y usuarios finales",

    distributorsTitle: "Para Distribuidores",
    distributorsDesc:
      "Programas para distribuidores con precios al por mayor y soporte de mercado. Asociaciones con proveedores líderes de China y América Latina.",
    distributorsFeature1: "Precios al por mayor con MOQ flexibles",
    distributorsFeature2: "Material de marketing y soporte de ventas",
    distributorsFeature3: "Soporte técnico y programas de capacitación",

    endUsersTitle: "Para Usuarios Finales",
    endUsersDesc:
      "Venta directa de cargadores EV para compradores individuales, empresas y operadores de flotas. Garantía de calidad y servicio post-venta incluidos.",
    endUsersFeature1: "Cargadores certificados con garantía completa",
    endUsersFeature2: "Paquetes de instalación disponibles",
    endUsersFeature3: "Soporte al cliente y servicio técnico",

    chargingOptionsTitle: "Opciones AC y DC",
    chargingOptionsDesc:
      "Oferta completa de soluciones de carga AC y DC. Desde cargadores residenciales de 7kW hasta cargadores rápidos DC de 350kW para implementaciones comerciales.",
    chargingOptionsFeature1: "Carga AC: 7kW - 22kW para uso diario",
    chargingOptionsFeature2: "Carga DC Rápida: 60kW - 180kW",
    chargingOptionsFeature3: "Carga DC Ultra-Rápida: 350kW para corredores de carga",

    // Deployment Scenarios
    scenariosTitle: "Escenarios de Implementación",
    scenariosSubtitle: "Soluciones adaptadas para cada tipo de ubicación e industria",

    mallsTitle: "Centros Comerciales",
    mallsDesc:
      "Aumente el tráfico de clientes y el tiempo de permanencia con carga EV de conveniencia. Gestión de carga inteligente para optimizar el uso de energía durante horas pico.",

    officesTitle: "Edificios de Oficinas",
    officesDesc:
      "Beneficios para empleados y visitantes con carga EV en el lugar. Integración con sistemas de facturación y control de acceso corporativo.",

    parkingTitle: "Instalaciones de Estacionamiento",
    parkingDesc:
      "Transforme estacionamientos en centros de ingresos con infraestructura de carga. Integración de pagos y monitoreo remoto incluidos.",

    residentialCommunitiesTitle: "Comunidades Residenciales",
    residentialCommunitiesDesc:
      "Infraestructura de carga compartida para edificios de apartamentos y condominios. Facturación individual por residente y gestión de energía.",

    // CTA
    ctaTitle: "¿Listo para Comenzar?",
    ctaSubtitle:
      "Póngase en contacto con nuestro equipo para discutir sus necesidades de infraestructura de carga EV y obtener una cotización personalizada.",
    getQuote: "Obtener Cotización",
    talkExpert: "Hablar con un Experto",

    // Footer
    footerTagline: "Soluciones completas de infraestructura de carga EV para América Latina.",
    footerSolutions: "Soluciones",
    footerInstallation: "Instalación",
    footerSales: "Ventas",
    footerScenarios: "Escenarios",
    footerCompany: "Empresa",
    footerHome: "Inicio",
    footerAbout: "Sobre Nosotros",
    footerContact: "Contacto",
    footerConnect: "Conectar",
    footerLinkedIn: "LinkedIn",
    footerEmail: "Correo",
    footerSupport: "Soporte",
    footerCopyright: "© 2025 EsLatin. Todos los derechos reservados.",
  },
  zh: {
    // Header
    home: "首页",
    solutions: "解决方案",
    contactUs: "联系我们",

    // Hero
    heroTitle: "全面的",
    heroTitleHighlight: "充电解决方案",
    heroSubtitle: "从安装到销售，我们为拉丁美洲的各种需求提供完整的充电基础设施解决方案。",

    // Installation Section
    installationTitle: "充电桩安装",
    installationSubtitle: "从住宅到大规模商业部署的专业安装服务",

    residentialTitle: "住宅安装",
    residentialDesc: "为电动汽车车主提供家庭充电器安装。即插即用解决方案，配备专业布线、安全认证和持续支持。",
    residentialFeature1: "单户和多户住宅安装",
    residentialFeature2: "2级充电器（7-22 kW）",
    residentialFeature3: "包括许可、认证和保修",

    commercialTitle: "商业安装",
    commercialDesc: "为企业、车队和公共场所提供商业充电站。可扩展的解决方案，从单站到多站点网络。",
    commercialFeature1: "2级和直流快充（最高350 kW）",
    commercialFeature2: "场地评估和电气设计",
    commercialFeature3: "负载管理软件集成",

    turnkeyTitle: "交钥匙交付",
    turnkeyDesc: "从规划到运营的完整交钥匙项目。我们处理许可、安装、调试和员工培训。",
    turnkeyFeature1: "完整的项目管理和时间表",
    turnkeyFeature2: "监管合规和认证",
    turnkeyFeature3: "安装后支持和维护",

    // Sales Section
    salesTitle: "充电桩销售",
    salesSubtitle: "为经销商和最终用户直接提供高质量充电桩",

    distributorsTitle: "经销商",
    distributorsDesc: "批发定价和市场支持的经销商计划。与中国和拉丁美洲领先供应商合作。",
    distributorsFeature1: "批发价格，MOQ灵活",
    distributorsFeature2: "营销材料和销售支持",
    distributorsFeature3: "技术支持和培训计划",

    endUsersTitle: "最终用户",
    endUsersDesc: "为个人买家、企业和车队运营商直销充电桩。包括质量保证和售后服务。",
    endUsersFeature1: "认证充电器，全面保修",
    endUsersFeature2: "提供安装套餐",
    endUsersFeature3: "客户支持和技术服务",

    chargingOptionsTitle: "交流和直流选项",
    chargingOptionsDesc: "完整的交流和直流充电解决方案。从7kW住宅充电器到350kW直流快充，用于商业部署。",
    chargingOptionsFeature1: "交流充电：7kW - 22kW，日常使用",
    chargingOptionsFeature2: "直流快充：60kW - 180kW",
    chargingOptionsFeature3: "直流超快充：350kW，用于充电走廊",

    // Deployment Scenarios
    scenariosTitle: "部署场景",
    scenariosSubtitle: "为每种位置和行业类型量身定制的解决方案",

    mallsTitle: "购物中心",
    mallsDesc: "通过便利充电增加客流量和停留时间。智能负载管理可在高峰时段优化能源使用。",

    officesTitle: "办公楼",
    officesDesc: "为员工和访客提供现场充电福利。与企业计费和访问控制系统集成。",

    parkingTitle: "停车设施",
    parkingDesc: "通过充电基础设施将停车场转变为收入中心。包括支付集成和远程监控。",

    residentialCommunitiesTitle: "住宅社区",
    residentialCommunitiesDesc: "为公寓楼和公寓提供共享充电基础设施。按居民单独计费和能源管理。",

    // CTA
    ctaTitle: "准备好开始了吗？",
    ctaSubtitle: "联系我们的团队讨论您的充电基础设施需求并获得定制报价。",
    getQuote: "获取报价",
    talkExpert: "与专家交谈",

    // Footer
    footerTagline: "拉丁美洲全面的充电基础设施解决方案。",
    footerSolutions: "解决方案",
    footerInstallation: "安装",
    footerSales: "销售",
    footerScenarios: "场景",
    footerCompany: "公司",
    footerHome: "首页",
    footerAbout: "关于我们",
    footerContact: "联系",
    footerConnect: "连接",
    footerLinkedIn: "领英",
    footerEmail: "邮箱",
    footerSupport: "支持",
    footerCopyright: "© 2025 EsLatin. 保留所有权利。",
  },
}
